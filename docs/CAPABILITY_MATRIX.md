...
| 可配置布局/输入预算 | 部分完成 | parse-only、max input、iterative engine budgets、layered pass budgets；DOT textual ID 通过 `NodeId` 索引 O(1) 反查；`zig build test-parse-scale` 使用 ReleaseFast 运行 10,000-node / 9,999-edge chain 与 4,096-node / 4,095-edge / 64-subgraph dense-attribute 两类 gate，每类限制 1 秒和 32 MiB parser arena，并报告 source/arena bytes 与 elapsed time；连续 cached 实测分别为 `5–6ms / 20.3MiB` 与 `6–8ms / 14.3MiB`；`zig build test-layout-render-scale` 同时覆盖 192 nodes / 388 edges / 4 subgraphs crossed layered graph 与 256 nodes / 512 edges sfdp graph，统一限制 layout 3 秒 / 8 MiB arena、SVG render 1 秒 / 4 MiB arena，验证对象分组和输出 hash，并通过归一化 `getrusage` 限制进程 peak RSS 为 96 MiB；连续实测 layered layout/render 为 `547–558ms / 0.61MiB`、`17–19ms / 0.19MiB`，sfdp 为 `8ms / 0.65MiB`、`1ms / 0.17MiB`，组合 peak RSS `3.98–4.15MiB`；所有当前引擎 `LayoutControl` / `LayoutWorkBudget` 取消和 CLI exit-2 smoke；sfdp Barnes-Hut 精度/交互次数门槛和 512-node SVG smoke | 增加更大规模及 neato/fdp/circo/twopi/packing 引擎基准门槛 |
