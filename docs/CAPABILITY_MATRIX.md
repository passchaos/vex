...
| force-directed 布局 | 部分完成 | independent neato stress-majorization；independent fdp spring-electrical with K/T0/len/weight and cluster boxes；deterministic Fruchterman-Reingold；能量下降、引擎区分和预算测试 | `sfdp` 仍需独立 multilevel engine 和语义/质量门槛 |
...
2. 独立的 `sfdp` multilevel engine，而不是 Fruchterman-Reingold 兼容别名。
3. 大图性能、内存和取消门槛。
