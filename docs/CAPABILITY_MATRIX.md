...
| --- | --- | --- | --- |
| DOT 实用语法 | 部分完成 | strict、named/anonymous scoped subgraph、ports、port-preserving node-list fanout、whitespace-free edge operators、edge chains、rank groups、Graphviz NAME/NUMBER lexical boundaries、`chkNum` ambiguous-number token split、balanced angle-string、BOM、字符串、注释与多 top-level graph stream 测试；Graphviz 七种 Latin-1 charset aliases 在 per-graph model boundary 原生转为 UTF-8，覆盖 graph/default/node/edge/subgraph/record-port text、UTF-8 no-op 和真实 `b56.gv` XML-valid SVG smoke；`parseDotGraphs` / `parseInputGraphs`、跨 graph 诊断、CLI 多 SVG 文档输出及 `src/testdata/dot_non_html_*.dot` corpus；Graphviz #2743 joined-attribute fixture 解析为 2 nodes / 1 edge，本机小于 256 KiB 的 786 个候选重扫中 708 个非 HTML DOT 直接通过、59 个 HTML-like label 排除、1 个 plain output 排除、4 个上游 malformed fixture 正确拒绝；14 个 Debug 并发超时样例去重为 8 个源 fixture 后，ReleaseFast parse-only 均为 0.00–0.01 秒、RSS 约 3.6–10.5 MiB | 继续扩展非 HTML-like label 的目标 DOT grammar corpus；补齐 Big-5 charset 转码或明确排除；所有支持/拒绝项有诊断测试 |
