...
| 可配置布局/输入预算 | 部分完成 | parse-only、max input、iterative engine budgets、layered pass budgets；DOT textual ID 通过 `NodeId` 索引 O(1) 反查；`zig build test-parse-scale` 使用 ReleaseFast 在固定 64 MiB arena 内解析 10,000 nodes / 9,999 edges 且限制为 1 秒；所有当前引擎 `LayoutControl` / `LayoutWorkBudget` 取消和 CLI exit-2 smoke；sfdp Barnes-Hut 精度/交互次数门槛和 512-node SVG smoke | 增加布局/render 的稳定时间、峰值内存观测和更大规模基准门槛 |
