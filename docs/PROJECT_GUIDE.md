...
- 更高的多图吞吐：Zig API 提供 `layoutGraphsParallel` 固定 worker batch 调度，保持输入顺序并为每项返回 `.layout` / `.failure`；每个 task 显式携带独立 allocator、graph 和 `LayoutConfig`，可通过原子 cancel flag 取消未开始及运行中的布局，单线程 target 自动退回串行执行。调度 allocator 只负责结果数组和 worker handle，不隐式共享 task allocator。CLI 对多 top-level graph stream 自动并行 layout，`--layout-workers` 控制并发度，SVG 仍按输入顺序串行输出，`--layout-work-budget` 按图独立计数。
