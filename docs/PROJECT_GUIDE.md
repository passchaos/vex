...
- 对大图友好：流式解析、可配置布局预算、并行布局阶段。CLI 已经提供 `--check` / `--validate` 作为 parse-only 验证入口，用于在 CI 或大图预览流程中只检查 DOT/Mermaid 输入并报告 graph/node/edge/subgraph 计数，不进入 layout/render；`--max-input-bytes` 作为 DOT/Mermaid 输入读取预算；DOT parser 的 textual ID 由 `NodeId` 直接索引反查，`zig build test-parse-scale` 会在 ReleaseFast 下分别生成带默认 `label="\N"` 的 10,000-node / 9,999-edge chain，以及包含 4,096 nodes / 4,095 attributed edges / 64 named subgraphs 的 structured workload；每类 gate 都限制为 1 秒、32 MiB parser arena，并报告 source bytes、arena bytes 和 elapsed milliseconds；`zig build test-layout-render-scale` 覆盖 layered、neato、fdp、Fruchterman-Reingold、sfdp、twopi、circo、patchwork 与 osage 全部原生布局族，当前使用 256-node iterative、192-node crossed layered、2,048-node sfdp/radial/circular 和 4,096-node packing workloads，在固定 arena、时间和 96 MiB normalized peak-RSS 预算内完成 layout/SVG，并验证对象分组计数和 output hash；独立 neato stress-majorization、独立 fdp spring-electrical、独立 sfdp multilevel spring-electrical 与 Fruchterman-Reingold 布局已经提供 `--layout-iterations` / `vex_layout_iterations` / `LayoutConfig.force.iterations` 作为迭代预算入口；所有布局引擎都支持 Zig `LayoutControl` / `LayoutWorkBudget` 取消，CLI 提供 `--layout-work-budget` 并在超限时以 exit code 2 退出；sfdp 细层斥力在大图上使用 deterministic Barnes-Hut quadtree，并有精度、交互次数和 512-node SVG smoke 门槛；layered/Sugiyama 布局已经提供 `--crossing-passes` / `--coordinate-passes`、`vex_crossing_passes` / `vex_coordinate_passes` 和 `LayoutConfig.layered` 作为 pass 预算入口，并支持 Graphviz `mclimit` 同时缩放 crossing MaxIter=24 与 MinQuit=8、`nslimit1` 按 node count 限制 rank network-simplex pivots、`searchsize` 限制每次 ordinary rank pivot 的负 cut-value 候选搜索、`nslimit` 按 node count 限制 coordinate refinement passes。
