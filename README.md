...
object groups with `data-vex-object-*` attributes for direct tooling hooks. DOT
can enable the same index with `graph [vex_svg_metadata=true]`, and the Zig API
can pass `.{ .svg = .{ .metadata = true } }`.
...
- Vex SVG output can optionally embed a machine-readable SVG metadata object index with layout/canvas facts, rendered object attributes, object geometry, waypoints, and layer metadata via `--svg-metadata`, `vex_svg_metadata=true`, or `SvgOptions.metadata`.
