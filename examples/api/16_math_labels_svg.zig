...
pub fn main(init: std.process.Init) !void {
    ...
    try graph.setGraphAttr(.{ .enable_math_label = true });
...
test "math label API example builds a graph with opt-in formula labels" {
    ...
    var graph = try vex.Graph.init(allocator, .{ .directed = true, .name = "MathLabels" });
    defer graph.deinit();
    const a = try graph.addNode("Energy $E=mc^2$", .{ .enable_math_label = true });
    const b = try graph.addNode("Ratio $\\frac{x}{y}$", .{ .enable_math_label = true });
    _ = try graph.addEdge(a, b, .{ .label = "cost $a^2$", .enable_math_label = true, .labelaligned = true });
