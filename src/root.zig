    ...
    pub fn setGraphAttr(self: *Graph, attr: GraphAttr) !void {
        switch (attr) {
            ...
            .ratio => |value| switch (value) {
                ...
                .fill => try self.setGraphAttrRaw("ratio", "fill"),
                .compress => try self.setGraphAttrRaw("ratio", "compress"),
...
fn parseShape(value: []const u8) Shape {
    ...
    if (std.ascii.eqlIgnoreCase(value, "assembly")) return .assembly;
...
pub fn renderSvg(writer: *Io.Writer, graph: *const Graph, layout: *const Layout, options: SvgOptions) Io.Writer.Error!void {
    ...
    const background_bottom = render_canvas_height - content_translate.y;
    ...
    const graph_wrap = try writeSvgInteractiveOpen(writer, graph.allocator, graph.attrs.items, .{ .graph_name = graph.name, .label_name = graph_fallback_title }, graph_fallback_title, graph_anchor_id);
...
fn renderSvgEdgeGroup(writer: *Io.Writer, graph: *const Graph, layout: *const Layout, edge_item: Edge, edge_routing: SvgEdgeRouting, concentrate: bool, layer: ?SvgLayerContext) Io.Writer.Error!void {
    ...
    const edge_wrap = try writeSvgInteractiveOpenKind(writer, graph.allocator, edge_item.attrs.items, .edge, edge_context, edgeFallbackTitle(edge_item, edge_context.edge_name), edge_anchor_id);
    if (edge_wrap == .none) {
...
fn renderSvgNodeGroup(writer: *Io.Writer, graph: *const Graph, layout: *const Layout, options: SvgOptions, node_item: Node, layer: ?SvgLayerContext) Io.Writer.Error!void {
    ...
    const node_wrap = try writeSvgInteractiveOpen(writer, graph.allocator, node_item.attrs.items, node_context, node_fallback_title, node_anchor_id);
    if (node_wrap == .none) {
...
const SvgAnchorIdOptions = struct {
    group: SvgGroupOpenOptions,
...
fn writeSvgInteractiveOpen(writer: *Io.Writer, allocator: std.mem.Allocator, attrs: []const Attr, context: LabelEscapeContext, fallback_title: ?[]const u8, anchor_id: ?SvgAnchorIdOptions) Io.Writer.Error!SvgInteractiveWrap {
    return writeSvgInteractiveOpenKind(writer, allocator, attrs, .default, context, fallback_title, anchor_id);
}
...
fn writeSvgInteractiveOpenKind(writer: *Io.Writer, allocator: std.mem.Allocator, attrs: []const Attr, kind: SvgInteractiveKind, context: LabelEscapeContext, fallback_title: ?[]const u8, anchor_id: ?SvgAnchorIdOptions) Io.Writer.Error!SvgInteractiveWrap {
    ...
    if (href) |target_href| {
        ...
        try writeSvgAnchorGroupOpen(writer, anchor_id);
        try writer.writeAll("<a href=\"");
        try writeXmlEscaped(writer, expanded_href);
        try writer.writeAll("\" xlink:href=\"");
        try writeXmlEscaped(writer, expanded_href);
        try writer.writeByte('"');
        if (expanded_tooltip) |tip| {
            try writer.writeAll(" xlink:title=\"");
...
fn writeSvgAnchorGroupOpen(writer: *Io.Writer, anchor_id: ?SvgAnchorIdOptions) Io.Writer.Error!void {
    try writer.writeAll("<g");
    if (anchor_id) |id_options| {
        try writer.writeAll(" id=\"a_");
        try writeSvgGroupId(writer, id_options.group);
        if (id_options.suffix) |suffix| {
            try writer.writeByte('-');
            try writeXmlEscaped(writer, suffix);
        }
        try writer.writeByte('"');
    }
    try writer.writeByte('>');
...
fn pathDataCommandCount(path_data: []const u8, command: u8) usize {
    return svg_mod.test_helpers.pathDataCommandCount(path_data, command);
}

fn svgPathCommandCount(svg: []const u8, command: u8) usize {
    return svg_mod.test_helpers.pathCommandCount(svg, command);
}

fn svgCubicSegmentCount(fragment: []const u8) usize {
    return svg_mod.test_helpers.cubicSegmentCount(fragment);
}

fn svgNumberAfter(fragment: []const u8, marker: []const u8) ?f64 {
    return svg_mod.test_helpers.numberAfter(fragment, marker);
}

fn svgGroupFragmentByTitle(svg: []const u8, title: []const u8) ?[]const u8 {
    return svg_mod.test_helpers.groupFragmentByTitle(svg, title);
}

fn svgGroupFragmentById(svg: []const u8, id: []const u8) ?[]const u8 {
    return svg_mod.test_helpers.groupFragmentById(svg, id);
}

fn svgFragmentHasDash(fragment: []const u8) bool {
    return svg_mod.test_helpers.fragmentHasDash(fragment);
}

fn svgRootFragment(svg: []const u8) ?[]const u8 {
    return svg_mod.test_helpers.rootFragment(svg);
}

const SvgTranslate = svg_mod.test_helpers.Translate;

const SvgViewBox = svg_mod.test_helpers.ViewBox;

fn svgViewBox(svg: []const u8) ?SvgViewBox {
    return svg_mod.test_helpers.viewBox(svg);
}

fn svgGraphvizTranslate(svg: []const u8) SvgTranslate {
    return svg_mod.test_helpers.graphvizTranslate(svg);
}
...
fn svgPolygonBBoxX(fragment: []const u8) ?f64 {
    return svg_mod.test_helpers.polygonBBoxX(fragment);
}

fn svgPolygonBBoxY(fragment: []const u8) ?f64 {
    return svg_mod.test_helpers.polygonBBoxY(fragment);
}

fn svgPolygonBBoxWidth(fragment: []const u8) ?f64 {
    return svg_mod.test_helpers.polygonBBoxWidth(fragment);
}

fn svgPolygonBBoxHeight(fragment: []const u8) ?f64 {
    return svg_mod.test_helpers.polygonBBoxHeight(fragment);
}

fn svgPolygonPointCount(fragment: []const u8) ?usize {
    return svg_mod.test_helpers.polygonPointCount(fragment);
}
...
fn svgNumbersInAttribute(fragment: []const u8, attr_name: []const u8, out: []f64) usize {
    return svg_mod.test_helpers.numbersInAttribute(fragment, attr_name, out);
}
...
fn graphFontNamesMode(graph: *const Graph) FontNames {
    const value = attrValue(graph.attrs.items, "fontnames") orelse return .gd;
    return svg_mod.font.parseNames(value) orelse .gd;
}

fn svgFont(graph: *const Graph, fontname: []const u8) SvgFont {
    return svg_mod.font.resolve(graphFontNamesMode(graph), fontname);
}
...
const SvgFont = svg_mod.font.Font;
...
fn renderSvgClusterBox(writer: *Io.Writer, graph: *const Graph, cluster: Subgraph, layout: *const Layout, index: usize, layer: ?SvgLayerContext) Io.Writer.Error!void {
    ...
    try writer.writeByte('\n');
    const cluster_anchor_id = SvgAnchorIdOptions{ .group = .{
        .graph = graph,
        .attrs = cluster.attrs.items,
        .default_id = default_id,
        .default_class = "cluster",
...
fn renderSvgNodeShape(writer: *Io.Writer, node_item: Node, layout: NodeLayout, visual: NodeVisual, options: SvgOptions) Io.Writer.Error!void {
    ...
    switch (node_item.shape) {
        ...
        .assembly => try renderSvgPolygonRings(8, writer, shape_layout, visual, assemblyPoints, diagonals),
...
fn terminatorPoints(layout: NodeLayout) [8]Point {
    const cx = layout.center.x;
    const left = layout.center.x - layout.width / 2.0;
    const right = layout.center.x + layout.width / 2.0;
    const top = layout.center.y - layout.height / 2.0;
    const bottom = layout.center.y + layout.height / 2.0;
    const stem_half = @min(layout.width * 0.18, layout.height * 0.35);
    const cap_top = top;
    const cap_bottom = top + layout.height * 0.36;
    const stem_bottom = bottom;
    return .{
        .{ .x = left, .y = cap_top },
        .{ .x = right, .y = cap_top },
        .{ .x = right, .y = cap_bottom },
        .{ .x = cx + stem_half, .y = cap_bottom },
        .{ .x = cx + stem_half, .y = stem_bottom },
        .{ .x = cx - stem_half, .y = stem_bottom },
        .{ .x = cx - stem_half, .y = cap_bottom },
        .{ .x = left, .y = cap_bottom },
    };
...
fn utrPoints(layout: NodeLayout) [6]Point {
    const cx = layout.center.x;
    const left = layout.center.x - layout.width / 2.0;
    const right = layout.center.x + layout.width / 2.0;
    const top = layout.center.y - layout.height / 2.0;
    const bottom = layout.center.y + layout.height / 2.0;
    const shoulder_y = top + layout.height * 0.28;
    const inset = @min(layout.width * 0.22, layout.height * 0.45);
    return .{
        .{ .x = left, .y = bottom },
        .{ .x = left, .y = shoulder_y },
        .{ .x = cx - inset, .y = top },
        .{ .x = cx + inset, .y = top },
        .{ .x = right, .y = shoulder_y },
        .{ .x = right, .y = bottom },
    };
}
...
fn assemblyPoints(layout: NodeLayout) [8]Point {
    const left = layout.center.x - layout.width / 2.0;
    const right = layout.center.x + layout.width / 2.0;
    const top = layout.center.y - layout.height / 2.0;
    const bottom = layout.center.y + layout.height / 2.0;
    const gap = layout.height * 0.16;
    const mid = layout.center.y;
    return .{
        .{ .x = left, .y = top },
        .{ .x = right, .y = top },
...
fn signaturePoints(layout: NodeLayout) [8]Point {
    const left = layout.center.x - layout.width / 2.0;
    const right = layout.center.x + layout.width / 2.0;
    const top = layout.center.y - layout.height / 2.0;
    const bottom = layout.center.y + layout.height / 2.0;
    const notch = @min(layout.width * 0.14, layout.height * 0.25);
    return .{
        .{ .x = left + notch, .y = top },
        .{ .x = right, .y = top },
        .{ .x = right, .y = bottom },
        .{ .x = left, .y = bottom },
        .{ .x = left, .y = top + notch },
        .{ .x = left + notch, .y = top + notch },
        .{ .x = left + notch, .y = bottom - notch },
        .{ .x = right - notch, .y = bottom - notch },
    };
}

fn insulatorPoints(layout: NodeLayout) [8]Point {
    const cx = layout.center.x;
    const cy = layout.center.y;
    const left = layout.center.x - layout.width / 2.0;
    const right = layout.center.x + layout.width / 2.0;
    const top = layout.center.y - layout.height / 2.0;
    const bottom = layout.center.y + layout.height / 2.0;
    const inset_x = @min(layout.width * 0.22, layout.height * 0.40);
    const inset_y = @min(layout.height * 0.22, layout.width * 0.18);
    return .{
        .{ .x = left, .y = top },
        .{ .x = right, .y = top },
        .{ .x = right, .y = bottom },
        .{ .x = left, .y = bottom },
        .{ .x = left + inset_x, .y = cy + inset_y },
        .{ .x = cx + inset_x, .y = cy + inset_y },
        .{ .x = cx + inset_x, .y = cy - inset_y },
        .{ .x = left + inset_x, .y = cy - inset_y },
    };
}

fn ribositePoints(layout: NodeLayout) [8]Point {
    const cx = layout.center.x;
    const cy = layout.center.y;
    const left = layout.center.x - layout.width / 2.0;
    const right = layout.center.x + layout.width / 2.0;
    const top = layout.center.y - layout.height / 2.0;
    const bottom = layout.center.y + layout.height / 2.0;
    const arm = @min(layout.width * 0.25, layout.height * 0.45);
    return .{
        .{ .x = cx - arm, .y = top },
        .{ .x = cx, .y = cy - arm * 0.35 },
        .{ .x = cx + arm, .y = top },
...
fn larrowPoints(layout: NodeLayout) [6]Point {
    const left = layout.center.x - layout.width / 2.0;
    const right = layout.center.x + layout.width / 2.0;
    const top = layout.center.y - layout.height / 2.0;
    const bottom = layout.center.y + layout.height / 2.0;
    const mid = layout.center.y;
    const head = @min(layout.width * 0.35, layout.height * 0.85);
    return .{
        .{ .x = left, .y = mid },
        .{ .x = left + head, .y = top },
        .{ .x = left + head, .y = top + layout.height * 0.25 },
        .{ .x = right, .y = top + layout.height * 0.25 },
        .{ .x = right, .y = bottom - layout.height * 0.25 },
        .{ .x = left + head, .y = bottom - layout.height * 0.25 },
    };
}

fn rarrowPoints(layout: NodeLayout) [6]Point {
    const left = layout.center.x - layout.width / 2.0;
    const right = layout.center.x + layout.width / 2.0;
    const top = layout.center.y - layout.height / 2.0;
    const bottom = layout.center.y + layout.height / 2.0;
    ...
    return .{
        .{ .x = left, .y = top + layout.height * 0.25 },
        .{ .x = right - head, .y = top + layout.height * 0.25 },
        .{ .x = right - head, .y = top },
        .{ .x = right, .y = mid },
        .{ .x = right - head, .y = bottom },
        .{ .x = right - head, .y = bottom - layout.height * 0.25 },
    };
}
...
fn writeSvgMarkerAttrs(writer: *Io.Writer, directed: bool, edge_id: EdgeId, visual: EdgeVisual) Io.Writer.Error!void {
    try svg_mod.marker.writeAttrs(writer, directed, edge_id, .{
        .start = visual.marker_start,
        .end = visual.marker_end,
        .scale = visual.marker_scale,
    });
}
...
fn markerEnabledByDir(dir: ?[]const u8, head: bool) bool {
    return svg_mod.marker.enabledByDir(dir, head);
}
...
test "code API sets typed node and edge options at creation" {
    ...
    const assembly = try graph.addNode("assembly", .{ .shape = .assembly });
    ...
    try std.testing.expectEqualStrings("terminator", attrValue(graph.nodes.items[terminator].attrs.items, "shape").?);
    try std.testing.expectEqual(Shape.utr, graph.nodes.items[utr].shape);
    ...
    try std.testing.expectEqual(Shape.assembly, graph.nodes.items[assembly].shape);
    try std.testing.expectEqualStrings("assembly", attrValue(graph.nodes.items[assembly].attrs.items, "shape").?);
    try std.testing.expectEqual(Shape.signature, graph.nodes.items[signature].shape);
    try std.testing.expectEqualStrings("signature", attrValue(graph.nodes.items[signature].attrs.items, "shape").?);
    try std.testing.expectEqual(Shape.insulator, graph.nodes.items[insulator].shape);
    ...
    try std.testing.expectEqualStrings("rarrow", attrValue(graph.nodes.items[rarrow].attrs.items, "shape").?);
...
test "SVG renderer honors graph label and bgcolor attributes" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, svg, "<g id=\"a_graph0\"><a href=\"https://example.com/graph-href\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, svg, "<a href=\"https://example.com/graph-href\" xlink:href=\"https://example.com/graph-href\" xlink:title=\"Graph tip\" target=\"_top\"><title>Graph tip</title>") != null);
...
test "SVG renderer honors Graphviz graph ratio output attributes" {
    const allocator = std.testing.allocator;
    var fill = try parseDot(allocator,
        \\digraph G {
        \\  graph [size="2,1", ratio=fill];
        \\  a -> b -> c;
        \\}
    );
    defer fill.deinit();

    var fill_layout = try layoutLayered(allocator, &fill, .{});
    defer fill_layout.deinit();
    const fill_svg = try renderSvgAlloc(allocator, &fill, &fill_layout, .{});
    defer allocator.free(fill_svg);
    const fill_view_box = svgViewBox(fill_svg) orelse return error.MissingViewBox;
    try std.testing.expectEqualStrings("fill", attrValue(fill.attrs.items, "ratio").?);
    try std.testing.expect(std.mem.indexOf(u8, fill_svg, "width=\"144pt\" height=\"72pt\"") != null);
    try std.testing.expectApproxEqAbs(@as(f64, 0.5), fill_view_box.height / fill_view_box.width, 0.001);

    var numeric = try Graph.init(allocator, .{ .directed = true });
    defer numeric.deinit();
    try numeric.setGraphAttr(.{ .ratio = .{ .value = 2.0 } });
    const numeric_canvas = graphSvgCanvas(&numeric, .{ .x = 100, .y = 50 });
    try std.testing.expectEqualStrings("2", attrValue(numeric.attrs.items, "ratio").?);
    try std.testing.expectApproxEqAbs(@as(f64, 100), numeric_canvas.view_box.x, 0.001);
    try std.testing.expectApproxEqAbs(@as(f64, 200), numeric_canvas.view_box.y, 0.001);
    try std.testing.expectApproxEqAbs(numeric_canvas.view_box.x, numeric_canvas.output.x, 0.001);
    try std.testing.expectApproxEqAbs(numeric_canvas.view_box.y, numeric_canvas.output.y, 0.001);

    var expand = try Graph.init(allocator, .{ .directed = true });
    defer expand.deinit();
    try expand.setGraphAttr(.{ .size = "4,4" });
    try expand.setGraphAttr(.{ .ratio = .expand });
    const expand_canvas = graphSvgCanvas(&expand, .{ .x = 96, .y = 48 });
    try std.testing.expectEqualStrings("expand", attrValue(expand.attrs.items, "ratio").?);
    try std.testing.expectApproxEqAbs(@as(f64, 288), expand_canvas.view_box.x, 0.001);
...
test "SVG renderer emits URL href and tooltip metadata" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, svg, "<a href=\"https://example.com/a\" xlink:href=\"https://example.com/a\" xlink:title=\"Node A\" target=\"_blank\">") != null);
    try std.testing.expect(std.mem.indexOf(u8, svg, "<title>Node A</title>") != null);
    try std.testing.expect(std.mem.indexOf(u8, svg, "<a href=\"https://example.com/e\" xlink:href=\"https://example.com/e\" xlink:title=\"Edge A to B\" target=\"_self\">") != null);
    try std.testing.expect(std.mem.indexOf(u8, svg, "<title>Edge A to B</title>") != null);
    try std.testing.expect(std.mem.indexOf(u8, svg, "<a href=\"https://example.com/cluster\" xlink:href=\"https://example.com/cluster\" xlink:title=\"Cluster API\" target=\"_parent\"><title>Cluster API</title>") != null);
    try std.testing.expect(std.mem.indexOf(u8, svg, "<title>API</title>\n<g id=\"a_clust1\"><a href=\"https://example.com/cluster\" xlink:href=\"https://example.com/cluster\" xlink:title=\"Cluster API\" target=\"_parent\">") != null);
    const node_anchor_href = std.mem.indexOf(u8, svg, "<a href=\"https://example.com/a\"") orelse return error.MissingNodeAnchor;
...
test "SVG renderer uses labels as URL tooltip fallback" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, svg, "<a href=\"https://example.com/graph\" xlink:href=\"https://example.com/graph\" xlink:title=\"Graph Label\"><title>Graph Label</title>") != null);
    try std.testing.expect(std.mem.indexOf(u8, svg, "<a href=\"https://example.com/a\" xlink:href=\"https://example.com/a\" xlink:title=\"a\"><title>a</title>") != null);
    try std.testing.expect(std.mem.indexOf(u8, svg, "<a href=\"https://example.com/cluster\" xlink:href=\"https://example.com/cluster\" xlink:title=\"API\"><title>API</title>") != null);
    try std.testing.expect(std.mem.indexOf(u8, svg, "<a href=\"https://example.com/e\" xlink:href=\"https://example.com/e\" xlink:title=\"edge label\"><title>edge label</title>") != null);
    try std.testing.expect(std.mem.indexOf(u8, svg, "<a href=\"https://example.com/e\" xlink:href=\"https://example.com/e\" xlink:title=\"external\"><title>external</title>") != null);
    try std.testing.expect(std.mem.indexOf(u8, svg, "<a href=\"https://example.com/e\" xlink:href=\"https://example.com/e\" xlink:title=\"head\"><title>head</title>") != null);
    try std.testing.expect(std.mem.indexOf(u8, svg, "<a href=\"https://example.com/e\" xlink:href=\"https://example.com/e\" xlink:title=\"tail\"><title>tail</title>") != null);
}
...
test "SVG renderer expands URL escape sequences with object context" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, svg, "<g id=\"a_edge1-label\"><a href=\"https://example.com/label/go/go/a-&gt;b\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, svg, "<g id=\"a_edge1-headlabel\"><a href=\"https://example.com/head/head/head/a-&gt;b\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, svg, "<g id=\"a_edge1-taillabel\"><a href=\"https://example.com/tail/tail/tail/a-&gt;b\"") != null);
...
test "SVG renderer honors edge label URL tooltip target metadata" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, inherited_svg, "<a href=\"https://example.com/inherited\" xlink:href=\"https://example.com/inherited\" xlink:title=\"Inherited edge\" target=\"_self\"><title>Inherited edge</title>") != null);
    try std.testing.expect(std.mem.indexOf(u8, inherited_svg, "<a href=\"https://example.com/inherited\" xlink:href=\"https://example.com/inherited\" xlink:title=\"main\" target=\"_self\"><title>main</title>") != null);
    try std.testing.expect(std.mem.indexOf(u8, inherited_svg, "<a href=\"https://example.com/inherited\" xlink:href=\"https://example.com/inherited\" xlink:title=\"external\" target=\"_self\"><title>external</title>") != null);
    try std.testing.expect(std.mem.indexOf(u8, inherited_svg, "<a href=\"https://example.com/inherited\" xlink:href=\"https://example.com/inherited\" xlink:title=\"head\" target=\"_self\"><title>head</title>") != null);
    try std.testing.expect(std.mem.indexOf(u8, inherited_svg, "<a href=\"https://example.com/inherited\" xlink:href=\"https://example.com/inherited\" xlink:title=\"tail\" target=\"_self\"><title>tail</title>") != null);
...
test "SVG renderer honors Graphviz fontnames modes" {
    const allocator = std.testing.allocator;
    ...
    defer explicit_svg.deinit();
    var explicit_svg_layout = try layoutLayered(allocator, &explicit_svg, .{});
    defer explicit_svg_layout.deinit();
    ...
    defer ps.deinit();
    var ps_layout = try layoutLayered(allocator, &ps, .{});
    defer ps_layout.deinit();
    const ps_svg = try renderSvgAlloc(allocator, &ps, &ps_layout, .{});
    defer allocator.free(ps_svg);
    ...
    defer gd.deinit();
    var gd_layout = try layoutLayered(allocator, &gd, .{});
    defer gd_layout.deinit();
...
test "SVG renderer honors edge label font position and decoration attributes" {
    ...
    const svg = try renderSvgAlloc(allocator, &graph, &layout, .{});
    defer allocator.free(svg);
    try std.testing.expect(std.mem.indexOf(u8, svg, "font-family=\"Courier,monospace\" font-size=\"18.00\"") != null);
...
test "SVG renderer honors DOT fontname and fontsize attributes" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, svg, "font-family=\"Courier,monospace\" font-size=\"18.00\">") != null);
    try std.testing.expect(std.mem.indexOf(u8, svg, "font-family=\"Courier,monospace\" font-size=\"22.00\"") != null);
...
test "cluster labels inherit root graph font attributes" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, inherited_fragment, "font-family=\"Courier,monospace\" font-size=\"24.00\"") != null);
...
test "DOT parser and SVG renderer support special Graphviz node shapes" {
    ...
    var graph = try parseDot(allocator,
        ...
        \\  assembly [label="Assembly", shape=assembly];
    ...
    try std.testing.expectEqual(Shape.assembly, graph.nodes.items[nodeIdByLabel(&graph, "assembly")].shape);
    ...
    try std.testing.expect(std.mem.indexOf(u8, svg, "Assembly") != null);
