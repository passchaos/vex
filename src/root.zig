...
fn graphvizCrossClusterLongRoute(layout: *const Layout, edge_item: Edge, rankdir: RankDir, route: EdgeRoute) EdgeRoute {
    ...
    var adjusted = route;
    const tail_shift: f64 = 1.0;
    const head_shift: f64 = -1.0;
...
test "user cluster example stays compact and Graphviz-like" {
    ...
    try std.testing.expect(distanceBetween(cross_control1, oracle_cross_control1) <= 1.9);
    try std.testing.expect(distanceBetween(cross_control2, oracle_cross_control2) <= 0.8);
    ...
    try expectSvgEdgeArrowTipNear(svg, graphviz_oracle, "a1-&gt;b3", 1.7);
