...
fn graphvizCrossClusterLongRoute(layout: *const Layout, edge_item: Edge, rankdir: RankDir, route: EdgeRoute) EdgeRoute {
    ...
    var adjusted = route;
    const tail_shift: f64 = 0.8;
    const head_shift: f64 = -0.8;
...
test "user cluster example stays compact and Graphviz-like" {
    ...
    try std.testing.expect(distanceBetween(cross_control1, oracle_cross_control1) <= 2.0);
    try std.testing.expect(distanceBetween(cross_control2, oracle_cross_control2) <= 0.9);
    ...
    try expectSvgEdgePathPointsNear(svg, graphviz_oracle, "a1-&gt;b3", 2.1);
    ...
    try expectSvgEdgeArrowTipNear(svg, graphviz_oracle, "a1-&gt;b3", 1.9);
